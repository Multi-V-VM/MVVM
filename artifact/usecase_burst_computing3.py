...
def plot_time(reu, aot_energy, aot_ps, aot1_energy, aot1_ps):
    ...
    sum_aot = exec_time[0] + exec_time[1] + exec_time[2] + exec_time[3]
    ...
    for idx, i in enumerate(exec_time1):
        ...
        if idx != len(exec_time1) - 1:
            time_spots1.append(time[idx + 5] - sum(exec_time1[idx + 1]) - base)
    sum_aot = [1 / x for x in sum_aot]
    avg_extended, percentile99_extended = get_avg_99percent(sum_aot, 1000)
    sum_aot1 = [1 / x for x in sum_aot1]
    avg_exec_time1, percentile99_extended1 = get_avg_99percent(sum_aot1, 100)
    # print("avg_exec_time1 ", exec_time1, "time_spots1 ", time[4])
    ax.plot(time_spots, avg_extended, "blue", label="Low priority")
    # ax.plot(time_spots, percentile99_extended, color="purple", linestyle="-")
    ax.plot(time_spots1, avg_exec_time1, "r", label="Native low priority")
    # ax.plot(time_spots1, percentile_99_exec_time1, color="pink", linestyle="-")
    # Add a legend to the plot
    ax.legend()
    print(average(exec_time[0]+exec_time[2])/ (1/average(sum_aot1)))
    print((1/average(sum_aot1))/average(exec_time[1]+exec_time[3]))
    print((1/average(sum_aot1))/average(exec_time[1]+exec_time[2]+exec_time[3]+exec_time[0]))
    print(average(exec_time[1]+exec_time[3]))
    print(average(exec_time[0]+exec_time[2]))
