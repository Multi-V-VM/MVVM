...
def plot_time(reu, aot_energy, aot_ps, aot1_energy, aot1_ps):
    ...
    for line in reu:
        try:
            if line.__contains__("Trial") and state < 5:
                ...
                # print("exec_time ",exec_time[-1])
                if to_append >= 0 and to_append < 1:
                    exec_time1[state - 6].append(to_append)
    ...
    for idx, i in enumerate(exec_time):
        ...
        if idx != len(exec_time) - 1:
            time_spots.append(time_spots[-1]+9)
    print(len(time))
    ...
    avg_exec_time1, percentile99_extended1 = get_avg_99percent(sum_aot1, 1)
    # print("avg_exec_time1 ", exec_time1, "time_spots1 ", time[4])
    ax.plot(time_spots, avg_extended, "blue", label="MVVM Edge+Cloud")
    # ax.plot(time_spots, percentile99_extended, color="purple", linestyle="-")
    ax.plot(time_spots1, avg_exec_time1, "r", label="Native Edge")
