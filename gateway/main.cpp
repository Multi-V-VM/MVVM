...
void packet_handler(u_char *user, const struct pcap_pkthdr *header, const u_char *packetptr) {
    ...
    if (iphdr->ip_v == 6) {
        ...
        case IPPROTO_TCP:
            ...
            SPDLOG_INFO( "TCP6  {}:{} -> {}:{}", srcip, ntohs(tcphdr->th_sport), dstip),
                                      ntohs(tcphdr->th_dport));
            SPDLOG_INFO( "ID:{} TOS:0x{}, TTL:{} IpLen:{} DgLen:{}", ntohs(iphdr->ip_id), iphdr->ip_tos),
        ...
        case IPPROTO_UDP:
            ...
            SPDLOG_INFO( "UDP6  {}:{} -> {}:{}", srcip, ntohs(udphdr->uh_sport), dstip),
        ...
        case IPPROTO_TCP:
            tcphdr = (struct tcphdr *)packetptr;
            SPDLOG_INFO( "TCP  {}:{} -> {}:{}", srcip, ntohs(tcphdr->th_sport), dstip),
                                      ntohs(tcphdr->th_dport));
            SPDLOG_INFO( "ID:{} TOS:0x{}, TTL:{} IpLen:{} DgLen:{}", ntohs(iphdr->ip_id), iphdr->ip_tos),
                                      iphdr->ip_ttl, 4 * iphdr->ip_hl, ntohs(iphdr->ip_len));
            SPDLOG_INFO( "{}{}{}{}{}{} Seq: 0x{} Ack: 0x{} Win: 0x{} TcpLen: {}"),
        ...
        case IPPROTO_UDP:
            udphdr = (struct udphdr *)packetptr;
            SPDLOG_INFO( "UDP  {}:{} -> {}:{}", srcip, ntohs(udphdr->uh_sport), dstip),
                                      ntohs(udphdr->uh_dport));
            SPDLOG_INFO( "ID:{} TOS:0x{}, TTL:{} IpLen:{} DgLen:{}", ntohs(iphdr->ip_id), iphdr->ip_tos),
                                      iphdr->ip_ttl, 4 * iphdr->ip_hl, ntohs(iphdr->ip_len));
        ...
        if (is_forward && !op_data->is_tcp) { // Skip the datalink layer header and get the IP header fields.
            ...
            for (auto [srcip_, destip_, new_srcip] : forward_pair) {
                SPDLOG_DEBUG(header->len, "srcip:", srcip_, " destip:", destip_);
        ...
        if ((rc = recv(client_fd, buffer, sizeof(buffer), 0)) > 0) {
            ...
            case MVVM_SOCK_SUSPEND_TCP_SERVER: {
                ...
                for (int idx = 0; idx < op_data->size; idx++) {
                    ...
                    SPDLOG_INFO( "server_ip:" , server_ip , ":" , server_port , " client_ip:" , client_ip , ":)"
                               , client_port;
