...
static bool read_leb(uint8 **p_buf, const uint8 *buf_end, uint32 maxbits, bool sign, uint64 *p_result, char *error_buf,
...
#define read_leb_int64(p, p_end, res)                                                                                  \
    do {                                                                                                               \
        uint64 res64;                                                                                                  \
        if (!read_leb((uint8 **)&p, p_end, 64, true, &res64, error_buf, error_buf_size))                               \
            goto fail;                                                                                                 \
        (res) = (int64)res64;                                                                                          \
    } while (0)
...
#define read_leb_uint32(p, p_end, res)                                                                                 \
    do {                                                                                                               \
        uint64 res64;                                                                                                  \
        if (!read_leb((uint8 **)&p, p_end, 32, false, &res64, error_buf, error_buf_size))                              \
            goto fail;                                                                                                 \
        (res) = (uint32)res64;                                                                                         \
    } while (0)
...
#define read_leb_int32(p, p_end, res)                                                                                  \
    do {                                                                                                               \
        uint64 res64;                                                                                                  \
        if (!read_leb((uint8 **)&p, p_end, 32, true, &res64, error_buf, error_buf_size))                               \
            goto fail;                                                                                                 \
        (res) = (int32)res64;                                                                                          \
    } while (0)
...
std::vector<std::unique_ptr<WAMRBranchBlock>> wasm_replay_csp_bytecode(WASMExecEnv *exec_env, WASMInterpFrame *frame,
    ...
    if (!(p <= target_addr && target_addr <= p_end)) {
    ...
    uint32 *frame_sp = frame_lp + cur_func->param_cell_num + cur_func->local_cell_num;
    uint8 value_type;
...
#define POP_I64() frame_sp -= 2;
    ...
    op_break_retry:
        ...
        case DEBUG_OP_BREAK: {
            ...
            uint64 offset = (p - 1) >= module_inst->module->load_addr ? (p - 1) - module_inst->module->load_addr : ~0;
            if (debug_instance) {
                if (wasm_debug_instance_get_obj_mem(debug_instance, offset, orignal_opcode, &size) && size == 1) {
