    ...
    switch (policy) {
        case SecurityPolicy::STRICT:
            ...
            pImpl->access_controls.push_back({
                "memory", {"read"}, 
                [](const std::string& op) { return op == "read"; }
            });
            break;
        case SecurityPolicy::BALANCED:
            ...
            pImpl->access_controls.push_back({
                "memory", {"read", "write"}, 
                [](const std::string& op) { return op == "read" || op == "write"; }
            });
            break;
...
bool SecurityFramework::authorizeMigration(const std::string& source, 
...
std::vector<uint8_t> SecurityFramework::encryptData(const void* data, size_t size,
    ...
    if (ctx.encryption_algo == CryptoAlgorithm::AES_256_GCM) {
        ...
        EVP_EncryptInit_ex(pImpl->cipher_ctx, EVP_aes_256_gcm(), nullptr, 
                          ctx.session_key.data(), ctx.nonce.data());
        
        // Encrypt
        EVP_EncryptUpdate(pImpl->cipher_ctx, encrypted.data(), &len, 
...
std::vector<uint8_t> SecurityFramework::decryptData(const void* encrypted_data, size_t size,
    ...
    if (ctx.encryption_algo == CryptoAlgorithm::AES_256_GCM && size > 16) {
        ...
        EVP_DecryptInit_ex(pImpl->cipher_ctx, EVP_aes_256_gcm(), nullptr,
                          ctx.session_key.data(), ctx.nonce.data());
        
        // Set tag
        EVP_CIPHER_CTX_ctrl(pImpl->cipher_ctx, EVP_CTRL_GCM_SET_TAG, 16,
                           const_cast<uint8_t*>(data + ciphertext_len));
        
        // Decrypt
        EVP_DecryptUpdate(pImpl->cipher_ctx, decrypted.data(), &len,
                         data, ciphertext_len);
        plaintext_len = len;
...
bool SecurityFramework::verifyIntegrity(const void* data, size_t size,
...
void SecurityFramework::updateSequenceNumber(SecurityContext& ctx, uint64_t new_seq) {
    ctx.sequence_number = new_seq;
}
...
bool SecurityFramework::checkAccess(const std::string& resource, const std::string& operation) {
    ...
    for (const auto& acl : pImpl->access_controls) {
        if (acl.resource == resource) {
            auto it = std::find(acl.allowed_operations.begin(), 
                               acl.allowed_operations.end(), operation);
...
std::vector<SecurityEvent> SecurityFramework::getSecurityEvents(
...
std::vector<uint8_t> SecurityFramework::secureSerialize(const void* state, size_t size,
                                                       const SecurityContext& ctx) {
...
bool SecurityFramework::secureDeserialize(const std::vector<uint8_t>& data, void* state,
                                         size_t size, const SecurityContext& ctx) {
    ...
    auto decrypted = decryptData(&*encrypted_start, 
                                 data.size() - SHA256_DIGEST_LENGTH, ctx);
...
WASISecurityExtensions::Capability WASISecurityExtensions::grantCapability(
...
bool WASISecurityExtensions::checkCapability(const Capability& cap, 
    ...
    return std::find(cap.permissions.begin(), cap.permissions.end(), operation) 
           != cap.permissions.end();
}
...
void WASISecurityExtensions::revokeCapability(const Capability& cap) {
    ...
    pImpl->active_capabilities.erase(
        std::remove_if(pImpl->active_capabilities.begin(),
                      pImpl->active_capabilities.end(),
                      [&cap](const Capability& c) {
                          return c.resource_type == cap.resource_type &&
                                 c.permissions == cap.permissions;
                      }),
        pImpl->active_capabilities.end()
    );
}
...
WASISecurityExtensions::SecureFdMigration 
WASISecurityExtensions::prepareSecureFdMigration(int fd) {
...
std::vector<uint8_t> MigrationAttestation::generateAttestationReport(
    const void* vm_state, size_t state_size) {
...
bool MigrationAttestation::verifyAttestationReport(const std::vector<uint8_t>& report,
    ...
    SHA256(static_cast<const unsigned char*>(expected_state), state_size, 
           expected_hash.data());
    
    return std::equal(report.begin(), report.begin() + SHA256_DIGEST_LENGTH,
                     expected_hash.begin());
}
...
std::vector<uint8_t> MigrationAttestation::createTPMQuote(
...
std::vector<uint8_t> MigrationAttestation::createSGXReport(const void* report_data, 
                                                          size_t size) {
...
std::vector<uint8_t> SecureMigrationProtocol::createHandshakeMessage() {
    ...
    EVP_PKEY_get_raw_public_key(pImpl->key_pair, 
                               message.data() + sizeof(version), &key_len);
...
std::vector<uint8_t> SecureMigrationProtocol::getSessionKey() const {
    return pImpl->session_key;
}

std::vector<uint8_t> SecureMigrationProtocol::prepareStateTransfer(const void* state, 
                                                                   size_t size) {
...
bool SecureMigrationProtocol::receiveStateTransfer(const std::vector<uint8_t>& data,
...
SecureMigrationProtocol::ProtocolState SecureMigrationProtocol::getCurrentState() const {
    return pImpl->state;
}
