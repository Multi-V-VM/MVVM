...
bool TFLiteGPUCCDelegate::initialize(const TFLiteGPUCCConfig& config) {
    ...
    pImpl->secure_memory_pool = std::make_unique<gpu::SecureGPUMemoryPool>(
        config.secure_memory_pool_size,
        gpu::MemoryType::SECURE
    );
    ...
    if (config.enable_secure_inference) {
        ...
        pImpl->tflite_gpu_options.inference_preference = 
            TFLITE_GPU_INFERENCE_PREFERENCE_SUSTAINED_SPEED;
        
        // Set priority for secure execution
        pImpl->tflite_gpu_options.inference_priority1 = 
            TFLITE_GPU_INFERENCE_PRIORITY_MIN_MEMORY_USAGE;
        pImpl->tflite_gpu_options.inference_priority2 = 
...
bool TFLiteGPUCCDelegate::loadSecureModel(const uint8_t* model_data, size_t model_size,
...
void* wasi_nn_tflite_gpu_cc_create() {
    return new TFLiteGPUCCDelegate();
}
...
int wasi_nn_tflite_gpu_cc_initialize(void* gpu_cc_ctx, int vendor,
...
int wasi_nn_tflite_gpu_cc_load_secure_model(void* gpu_cc_ctx,
                                           const uint8_t* model_data,
                                           size_t model_size,
                                           void** secure_model_ptr,
    ...
    auto* delegate = static_cast<TFLiteGPUCCDelegate*>(gpu_cc_ctx);
    return delegate->loadSecureModel(model_data, model_size, 
                                    secure_model_ptr, secure_model_size) ? 0 : -1;
}

void wasi_nn_tflite_gpu_cc_free_secure_model(void* gpu_cc_ctx,
                                            void* secure_model_ptr) {
    if (!gpu_cc_ctx) {
        return;
    }
...
int wasi_nn_tflite_gpu_cc_get_attestation(void* gpu_cc_ctx,
