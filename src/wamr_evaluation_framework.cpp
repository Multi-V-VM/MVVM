...
MigrationOverheadBenchmark::MigrationOverheadBenchmark() 
...
BenchmarkResult MigrationOverheadBenchmark::run() {
    ...
    for (size_t i = 0; i < pImpl->config.iterations; i++) {
        ...
        auto restore_duration = std::chrono::duration_cast<std::chrono::microseconds>(
    ...
    // Calculate results
    double avg_checkpoint_time = std::accumulate(checkpoint_times.begin(), 
                                               checkpoint_times.end(), 0.0) / checkpoint_times.size();
    double avg_restore_time = std::accumulate(restore_times.begin(), 
                                            restore_times.end(), 0.0) / restore_times.size();
    size_t avg_data_size = std::accumulate(data_sizes.begin(), 
                                          data_sizes.end(), 0UL) / data_sizes.size();
    
    result.execution_time_ms = avg_checkpoint_time + avg_restore_time;
    result.throughput_mbps = (avg_data_size / (1024.0 * 1024.0)) / 
    ...
    result.custom_metrics["compression_ratio"] = 
        (double)avg_data_size / pImpl->state_size;
...
void MigrationOverheadBenchmark::setStateSize(size_t size) {
    pImpl->state_size = size;
}

void MigrationOverheadBenchmark::setMigrationStrategy(MigrationStrategy strategy) {
...
SecurityOverheadBenchmark::SecurityOverheadBenchmark() 
    : pImpl(std::make_unique<Impl>()) {
...
BenchmarkResult SecurityOverheadBenchmark::run() {
    ...
    for (size_t i = 0; i < pImpl->config.iterations; i++) {
        ...
        auto encrypted = pImpl->security_framework->encryptData(
            pImpl->test_data.data(), pImpl->test_data.size(), security_ctx);
        auto enc_end = std::chrono::high_resolution_clock::now();
        
        auto enc_duration = std::chrono::duration_cast<std::chrono::microseconds>(
            enc_end - enc_start);
        encryption_times.push_back(enc_duration.count() / 1000.0);
        
        // Decryption benchmark
        auto dec_start = std::chrono::high_resolution_clock::now();
        auto decrypted = pImpl->security_framework->decryptData(
            encrypted.data(), encrypted.size(), security_ctx);
        ...
        auto integrity = pImpl->security_framework->createIntegrityCheck(
            pImpl->test_data.data(), pImpl->test_data.size());
        pImpl->security_framework->verifyIntegrity(
            pImpl->test_data.data(), pImpl->test_data.size(), integrity);
        auto int_end = std::chrono::high_resolution_clock::now();
        
        auto int_duration = std::chrono::duration_cast<std::chrono::microseconds>(
            int_end - int_start);
        integrity_times.push_back(int_duration.count() / 1000.0);
    }
    
    // Calculate results
    double avg_encryption_time = std::accumulate(encryption_times.begin(), 
                                               encryption_times.end(), 0.0) / encryption_times.size();
    double avg_decryption_time = std::accumulate(decryption_times.begin(), 
                                               decryption_times.end(), 0.0) / decryption_times.size();
    double avg_integrity_time = std::accumulate(integrity_times.begin(), 
                                              integrity_times.end(), 0.0) / integrity_times.size();
    
    result.execution_time_ms = avg_encryption_time + avg_decryption_time + avg_integrity_time;
    result.throughput_mbps = (pImpl->data_size / (1024.0 * 1024.0)) / 
                            (avg_encryption_time / 1000.0);
    result.memory_usage_bytes = getCurrentMemoryUsage();
    result.cpu_utilization_percent = getCurrentCPUUsage();
...
void SecurityOverheadBenchmark::setSecurityPolicy(security::SecurityPolicy policy) {
    pImpl->policy = policy;
}

void SecurityOverheadBenchmark::setDataSize(size_t size) {
    pImpl->data_size = size;
}
...
GPUPerformanceBenchmark::GPUPerformanceBenchmark() 
    : pImpl(std::make_unique<Impl>()) {
...
void GPUPerformanceBenchmark::setup(const BenchmarkConfig& config) {
    ...
    if (pImpl->use_cc) {
        std::vector<gpu::CCFeature> features = {
    ...
    pImpl->gpu_framework->registerSecureKernel(pImpl->kernel_name, 
                                             dummy_kernel_code.data(), 
                                             dummy_kernel_code.size());
}
...
BenchmarkResult GPUPerformanceBenchmark::run() {
    ...
    void* gpu_input = pImpl->gpu_framework->getInterface()->allocateSecureMemory(
        data_size, gpu::MemoryType::DEVICE_LOCAL);
    void* gpu_output = pImpl->gpu_framework->getInterface()->allocateSecureMemory(
        data_size, gpu::MemoryType::DEVICE_LOCAL);
    ...
    for (size_t i = 0; i < pImpl->config.iterations; i++) {
        ...
        pImpl->gpu_framework->secureDataTransferToGPU(
            host_input.data(), gpu_input, data_size, pImpl->use_cc);
        ...
        pImpl->gpu_framework->runSecureKernel(pImpl->kernel_name, args, 
    ...
    // Calculate results
    double avg_transfer_time = std::accumulate(transfer_times.begin(), 
                                             transfer_times.end(), 0.0) / transfer_times.size();
    double avg_kernel_time = std::accumulate(kernel_times.begin(), 
                                           kernel_times.end(), 0.0) / kernel_times.size();
...
void GPUPerformanceBenchmark::cleanup() {
    pImpl->gpu_framework->shutdown();
}

bool GPUPerformanceBenchmark::validate(const BenchmarkResult& result) {
    return result.execution_time_ms > 0 && 
           result.custom_metrics.at("gflops") > 0;
}

void GPUPerformanceBenchmark::setKernelType(const std::string& kernel_name) {
    pImpl->kernel_name = kernel_name;
}

void GPUPerformanceBenchmark::setProblemSize(size_t size) {
    pImpl->problem_size = size;
}

void GPUPerformanceBenchmark::setUseConfidentialComputing(bool use_cc) {
    pImpl->use_cc = use_cc;
}
...
void BenchmarkSuite::setConfig(const BenchmarkConfig& config) {
    pImpl->config = config;
}

void BenchmarkSuite::addBaseline(const Baseline& baseline) {
    pImpl->baselines[baseline.name] = baseline;
}
...
void BenchmarkSuite::runAll() {
    ...
    for (auto& benchmark : pImpl->benchmarks) {
        ...
        if (benchmark->validate(result)) {
            ...
            SPDLOG_INFO("Benchmark {} completed: {} ms", 
                       result.benchmark_name, result.execution_time_ms);
        } else {
...
std::vector<BenchmarkResult> BenchmarkSuite::getResults() const {
    return pImpl->results;
}

std::vector<BenchmarkResult> BenchmarkSuite::getResultsByCategory(
...
void BenchmarkSuite::compareWithBaseline(const std::string& baseline_name) {
    ...
    for (const auto& result : pImpl->results) {
        ...
        if (baseline_time_it != baseline.metrics.end()) {
            double speedup = calculateSpeedup(baseline_time_it->second, 
...
void BenchmarkSuite::exportResults(const std::string& format, 
...
BenchmarkSuite::Statistics BenchmarkSuite::calculateStatistics(
