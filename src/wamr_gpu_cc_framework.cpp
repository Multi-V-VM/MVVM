...
struct GPUCCFramework::Impl {
    ...
    void startTiming() { last_operation_start = std::chrono::high_resolution_clock::now(); }

    double endTiming() {
        auto end = std::chrono::high_resolution_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - last_operation_start);
...
std::vector<GPUDevice> NVIDIAGPUCCImpl::enumerateDevices() {
    ...
    device.cc_features = {CCFeature::MEMORY_ENCRYPTION, CCFeature::SECURE_BOOT, CCFeature::REMOTE_ATTESTATION,
...
GPUDevice NVIDIAGPUCCImpl::getCurrentDevice() { return pImpl->current_device; }

bool NVIDIAGPUCCImpl::initializeCC(const std::vector<CCFeature> &required_features) {
    // Check if device supports required features
    for (const auto &feature : required_features) {
        auto it =
            std::find(pImpl->current_device.cc_features.begin(), pImpl->current_device.cc_features.end(), feature);
...
bool NVIDIAGPUCCImpl::executeSecureKernel(const std::string &kernel_name, void **args, size_t num_args,
    ...
    SPDLOG_INFO("Executing secure kernel {} with grid={}, block={}", kernel_name, grid_size, block_size);
...
AMDGPUCCImpl::AMDGPUCCImpl() : pImpl(std::make_unique<Impl>()) { SPDLOG_INFO("AMD GPU CC implementation initialized"); }
...
GPUDevice AMDGPUCCImpl::getCurrentDevice() { return pImpl->current_device; }
...
bool AMDGPUCCImpl::executeSecureKernel(const std::string &kernel_name, void **args, size_t num_args, size_t grid_size,
                                       size_t block_size) {
    auto it = pImpl->kernel_handles.find(kernel_name);
    if (it == pImpl->kernel_handles.end()) {
    ...
    SPDLOG_DEBUG("Grid size: {}, Block size: {}, Args: {}", grid_size, block_size, num_args);
...
GPUDevice IntelGPUCCImpl::getCurrentDevice() { return pImpl->current_device; }
...
bool IntelGPUCCImpl::executeSecureKernel(const std::string &kernel_name, void **args, size_t num_args, size_t grid_size,
                                         size_t block_size) {
    auto it = pImpl->kernel_handles.find(kernel_name);
    if (it == pImpl->kernel_handles.end()) {
        SPDLOG_ERROR("Kernel '{}' not found", kernel_name);
...
void SecureGPUMemoryPool::defragment() {
    ...
    for (size_t i = 0; i < pImpl->free_blocks.size() - 1;) {
        if (pImpl->free_blocks[i].first + pImpl->free_blocks[i].second == pImpl->free_blocks[i + 1].first) {
            // Merge blocks
...
void GPUCCScheduler::waitForCompletion() {
    ...
    pImpl->queue_cv.wait(lock, [this] { return pImpl->task_queue.empty() && pImpl->current_tasks == 0; });
}
