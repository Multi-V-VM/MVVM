...
private:
    ...
    public:
        NetworkCondition get_current_conditions() {
            ...
            cout << "[Network] Current: " << condition.bandwidth_mbps << "Mbps, "
                 << condition.latency_ms << "ms latency, "
                 << condition.packet_loss * 100 << "% loss" << endl;
    ...
    private:
        map<DeviceType, ComputeCapability> capabilities = {
            {DeviceType::RASPBERRY_PI, {4, 4, false, false, 0}},
    ...
    public:
        ComputeCapability get_capability(DeviceType type) {
        ...
        DeviceType find_suitable_device(const WorkloadProfile& workload) {
            ...
            vector<DeviceType> candidates = {
                DeviceType::RASPBERRY_PI,
                DeviceType::NVIDIA_JETSON,
            ...
            for (auto device : candidates) {
                ...
                if (workload.memory_usage_mb <= cap.memory_gb * 1024 &&
                    (!workload.requires_gpu || cap.has_gpu) &&
    ...
    public:
        ...
        bool should_migrate(const NetworkCondition& network, 
                          const ComputeCapability& current,
                          const ComputeCapability& target,
            ...
            cout << "[Orchestrator] Speedup: " << speedup << "x, Migration cost: " 
                 << migration_cost << "ms" << endl;
            
            // Consider deadline
            bool worth_migrating = speedup > 2.0 && 
                                 migration_cost < workload.deadline.count() * 0.1;
        ...
        void execute_migration(DeviceType from, DeviceType to, const WorkloadProfile& workload) {
            cout << "\n[Migration] Starting migration from device " << static_cast<int>(from)
                 << " to device " << static_cast<int>(to) << endl;
...
public:
    IoTEdgeAgent(DeviceType initial_device = DeviceType::RASPBERRY_PI) 
    ...
    ~IoTEdgeAgent() {
        ...
        if (gpu_cc_framework) wamr_gpu_cc_framework_destroy(gpu_cc_framework);
    ...
    void process_sensor_data(const string& data_stream) {
        ...
        if (target_device != current_device) {
            ...
            if (orchestrator.should_migrate(current_network,
                                          registry.get_capability(current_device),
                                          registry.get_capability(target_device),
                                          workload)) {
    ...
    void perform_live_migration(DeviceType target, const WorkloadProfile& workload) {
        ...
        if (wamr_migration_create_checkpoint(migration_optimizer, nullptr, 
            ...
            if (wamr_encrypt_data(security_framework, checkpoint_data, checkpoint_size,
                ...
                cout << "[Transfer] Migrating to " << static_cast<int>(target) 
                     << " (ETA: " << transfer_time_ms << "ms)" << endl;
    ...
    void demonstrate_predictive_migration() {
        ...
        if (monitor.predict_network_failure(network_history)) {
            ...
            WorkloadProfile critical_workload{
                "critical_monitoring",
                5,
                256,
                false,
    ...
    void demonstrate_edge_cloud_continuum() {
        ...
        vector<WorkloadProfile> workloads = {
            {"lightweight_filter", 2, 128, false, milliseconds(1000)},
    ...
    void run_demo() {
        ...
        cout << "Predictive migration: " << (migration_strategy.enable_predictive_migration ? "ENABLED" : "DISABLED") << endl;
