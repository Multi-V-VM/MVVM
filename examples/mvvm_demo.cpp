...
int main(int argc, char* argv[]) {
    ...
    std::cout << "--------------------------\n";
    {
        ...
        auto encrypted = security.encryptData(sensitive_data.data(), 
                                            sensitive_data.size(), ctx);
        std::cout << "Data encrypted: " << encrypted.size() << " bytes\n";
        
        // Decrypt data
        auto decrypted = security.decryptData(encrypted.data(), 
                                            encrypted.size(), ctx);
    ...
    std::cout << "----------------------------------\n";
    {
        ...
        if (gpu_framework.initialize(gpu::GPUVendor::NVIDIA)) {
            ...
            if (gpu_framework.setupSecureComputation(features)) {
                ...
                gpu_framework.registerSecureKernel("demo_kernel", 
                                                  dummy_kernel.data(), 
                                                  dummy_kernel.size());
