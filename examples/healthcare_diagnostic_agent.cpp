...
enum class ExecutionLocation { LOCAL_DEVICE, EDGE_SERVER, PRIVATE_CLOUD, PUBLIC_CLOUD };
    ...
    struct SchedulingPolicy {
        map<DataSensitivity, vector<ExecutionLocation>> allowed_locations = {
            ...
            {DataSensitivity::MEDIUM,
             {ExecutionLocation::LOCAL_DEVICE, ExecutionLocation::EDGE_SERVER, ExecutionLocation::PRIVATE_CLOUD}},
            {DataSensitivity::LOW,
             {ExecutionLocation::LOCAL_DEVICE, ExecutionLocation::EDGE_SERVER, ExecutionLocation::PRIVATE_CLOUD,
              ExecutionLocation::PUBLIC_CLOUD}}};
...
public:
    HealthcareDiagnosticAgent() {
        ...
        migration_optimizer->setDirtyPageThreshold(10000); // Less frequent for stability
    ...
    ExecutionLocation schedule_workload(const string &medical_data) {
        ...
        cout << "  Edge: CPU=" << edge_res.cpu_percent << "%, Mem=" << edge_res.memory_mb
             << "MB, GPU=" << edge_res.gpu_available << endl;
        cout << "  Cloud: CPU=" << cloud_res.cpu_percent << "%, Mem=" << cloud_res.memory_mb
             << "MB, GPU=" << cloud_res.gpu_available << endl;
    ...
    string process_medical_image(const string &image_data, const string &patient_info) {
        ...
        if (privacy_config.enable_homomorphic_encryption && location == ExecutionLocation::PUBLIC_CLOUD) {
    ...
    void migrate_to_edge() {
        ...
        try {
            ...
            security::SecurityContext ctx = security_framework->createSecurityContext("edge-server");
    ...
    void migrate_to_private_cloud() {
        ...
        bool attestation_verified = security_framework->establishSecureChannel("private-cloud.hospital.local", 443);
    ...
    void run_demo() {
        ...
        cout << "Differential Privacy: " << (privacy_config.enable_differential_privacy ? "ENABLED" : "DISABLED")
             << endl;
        cout << "Homomorphic Encryption: " << (privacy_config.enable_homomorphic_encryption ? "ENABLED" : "DISABLED")
             << endl;
